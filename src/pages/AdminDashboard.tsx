                           ...
                           <td className="px-6 py-4">
                             <div className="flex items-center gap-2">
                               <button 
                                 onClick={() => {
                                   const planUpper = String(key.plan_name || "").toUpperCase();
                                   const targetUrl = planUpper.includes("TELEGRAM") ? `https://tracexdata-api.onrender.com/api/telegram?key=${key.api_key}&api=gaurav_beniwal_0001` : planUpper.includes("VEHICLE") ? `https://tracexdata-api.onrender.com/api/vehicle?key=${key.api_key}&query=BR07PB6268` : (planUpper.includes("PAN") || planUpper.includes("PN")) ? `https://tracexdata-api.onrender.com/api/pancard?key=${key.api_key}&query=NTEPK1628C` : (planUpper.includes("ADHR") || planUpper.includes("IDENTITY") || planUpper.includes("AADH")) ? `https://tracexdata-api.onrender.com/api/identity?key=${key.api_key}&query=381933049732` : (planUpper.includes("BNK") || planUpper.includes("BANK") || planUpper.includes("BA&NK")) ? `https://tracexdata-api.onrender.com/api/bank?key=${key.api_key}&query=ABCD0001325` : `https://tracexdata-api.onrender.com/api/lookup?key=${key.api_key}&number=9879712345`;
                                   navigator.clipboard.writeText(targetUrl);
                                   alert('Full API URL Copied!');
                                 }}
